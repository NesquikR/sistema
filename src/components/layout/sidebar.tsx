...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-line bg-surface/70 backdrop-blur-2xl lg:flex">
      {/* Marca */}
      <div className="flex h-[60px] items-center gap-2.5 border-b border-line px-5">
        <div className="relative flex h-7 w-7 items-center justify-center rounded-[9px] bg-[linear-gradient(135deg,#8b5cf6,#ec4899)] shadow-[0_6px_18px_-8px_#8b5cf6]">
        ...
        <div className="leading-none">
          <div className="text-[14px] font-semibold tracking-[-0.02em] text-fg">
            Beauty<span className="text-violet-soft">Bot</span>
          </div>
          <div className="mt-1 text-[10px] font-medium uppercase tracking-[0.14em] text-fg-subtle">
            Intelligence
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-line p-3">
        <div className="rounded-[12px] border border-line bg-black/25 p-3">
...
function NavLink({
  ...
  return (
    <Link
      href={item.href}
      className={cn(
        "group relative flex h-9 items-center gap-2.5 rounded-[10px] px-2.5 text-[13px] transition-colors duration-150",
    ...
    >
      {active && (
        <motion.span
          ...
          className="absolute inset-0 -z-10 rounded-[10px] border border-line-strong bg-white/[0.06] shadow-[0_1px_0_0_#ffffff10_inset]"
        />
      )}
      {active && (
        <span className="absolute -left-3 top-1/2 h-4 w-[2px] -translate-y-1/2 rounded-full bg-[linear-gradient(180deg,#8b5cf6,#ec4899)]" />
      )}
      <Icon
        className={cn(
          "h-4 w-4 shrink-0 transition-colors",
          active ? "text-violet-soft" : "text-fg-subtle group-hover:text-fg-muted",
        )}
        strokeWidth={1.9}
      />
      <span className="flex-1 truncate font-medium">{item.label}</span>
      {item.badge && (
        <span className="num rounded-full border border-violet/25 bg-violet/12 px-1.5 py-[1px] text-[10px] font-semibold text-violet-soft">
          {item.badge}
        </span>
      )}
    </Link>
