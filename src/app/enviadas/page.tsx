  ...
  return (
    <PageShell>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3.5 lg:grid-cols-4">
        <Metric icon={Send} label="Publicações" value={int(published.length)} hint="últimas 24 h" color="var(--color-primary)" />
        <Metric icon={MousePointerClick} label="Cliques" value={compact(clicks)} hint={`CTR ${pct(7.4)}`} color="var(--color-primary)" />
        <Metric icon={CheckCheck} label="Conversões" value={int(conversions)} hint={`taxa ${pct((conversions / clicks) * 100)}`} color="#34d399" />
        <Metric icon={TrendingUp} label="Receita atribuída" value={money(revenue)} hint="comissão estimada" color="var(--color-danger)" />
      </div>
        ...
        <Card>
          ...
          <CardBody className="space-y-2.5">
            {channels.map((c) => (
              <div
                key={c.id}
                className="rounded-[13px] border border-line bg-surface-2 p-3.5 transition-colors hover:border-white/[0.13]"
              >
        ...
        <CardBody className="space-y-2.5">
          {rows.map((d, i) => (
            <motion.div
              ...
              className="group grid grid-cols-1 gap-4 rounded-[14px] border border-line bg-surface-2 p-4 transition-colors duration-200 hover:border-white/[0.13] lg:grid-cols-[minmax(0,1fr)_300px]"
            >
              <div className="flex gap-3.5">
                ...
                <div className="min-w-0">
                  ...
                  <pre className="mt-2.5 whitespace-pre-wrap rounded-[10px] border border-line bg-surface-2 px-3 py-2.5 font-mono text-[11.5px] leading-relaxed text-fg-muted">
                    {d.message}
                  </pre>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 self-start lg:border-l lg:border-line lg:pl-4">
                <Cell label="Cliques" value={int(d.clicks ?? 0)} color="var(--color-primary)" />
                <Cell label="Conversões" value={int(d.conversions ?? 0)} color="#34d399" />
                <Cell label="Receita" value={money(d.revenue ?? 0)} color="var(--color-danger)" />
              </div>
            </motion.div>
          ))}
        </CardBody>
